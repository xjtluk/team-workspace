...
3. CC 的消息：如果他 @ 了你，你必须回复；如果没 @ 你但内容涉及你的领域（需求、产品、排期），也可以回复
4. CC @ 了别人（不是你）的消息：不要回复
5. 回复时如果内容涉及 CC，用 @CC 开头，这样他知道这条消息是给他的
6. 同一件事只回复一次
7. 不要用"好的"、"收到"开头`;
...
async function loadHistory() {
  try {
    ...
    if (data.messages) {
      ...
      console.log(`[小马] 加载了 ${data.messages.length} 条历史消息`);
    }
  } catch (err) {
    console.error('[小马] 加载历史失败:', err.message);
  }
}
await loadHistory();
...
ws.on('message', async (raw) => {
  ...
  if (msg.from !== 'kk' && /@CC/i.test(msg.content) && !/@(小马|xiaoma)/i.test(msg.content)) {
    console.log(`[小马] 跳过: ${msg.fromName} @ 了CC`);
    return;
  }
  ...
  const prompt = `群聊记录：\n${recent}\n\n${msg.fromName} 刚说："${msg.content}"\n\n你是小马（产品Leader）。判断你需要回复吗？不需要就回复 [SKIP]。需要就直接回复内容，如果涉及CC就用 @CC 开头。`;
