...
// 心跳超时：优先读取环境变量，方便不改代码调整
const HEARTBEAT_TIMEOUT = parseInt(process.env.HEARTBEAT_TIMEOUT_MS || '60000', 10);
...
router.post("/", (req, res) => {
  try {
    ...
    // 心跳只保活，不盲目重置状态
    // 只有 agent 之前是 offline/error 时才重置为 idle（首次上线）
    // working/talking/thinking 等状态由 agent 自己管理
    const wasOffline = agent.current_status === "offline" || agent.current_status === "error" || !agent.online;
    ...
    // 查询新消息（自上次心跳以来），心跳作为补充通道，WS 是主通道
    let newMessages = [];
    ...
    console.error('[heartbeat] 内部错误:', err.message);
  ...
  setInterval(() => {
    try {
      ...
      offlineAgents.forEach(agent => {
        run("UPDATE agents SET online = 0, current_status = ?, current_activity = ? WHERE id = ?", ["error", "异常: 心跳超时", agent.id]);
        broadcast({
          type: "agent_offline",
          payload: { agentId: agent.id, online: false, status: "error", activity: "异常: 心跳超时" },
        });
        console.log(`[Heartbeat] Agent ${agent.id} marked ERROR (no heartbeat for ${HEARTBEAT_TIMEOUT / 1000}s)`);
      });
    } catch (err) {
